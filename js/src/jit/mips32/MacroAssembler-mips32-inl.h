...
{
    as_sltiu(ScratchRegister, dest.low, imm.value & LOW_32_MASK);
    ma_subu(dest.low, dest.low, Imm32(imm.value & LOW_32_MASK));
    ma_subu(dest.high, dest.high, Imm32((imm.value >> 32) & LOW_32_MASK));
    as_subu(dest.high, dest.high, ScratchRegister);
}
...
MacroAssembler::lshift64(Register unmaskedShift, Register64 dest)
{
    ...
    ma_sll(ScratchRegister, dest.low, shift);
    as_or(dest.high, dest.high, ScratchRegister);
    ma_negu(shift, shift);
    ma_srl(ScratchRegister, dest.low, shift);
    as_or(dest.high, dest.high, ScratchRegister);
    ma_and(shift, unmaskedShift, Imm32(0x3f));
...
MacroAssembler::rshift64Arithmetic(Register unmaskedShift, Register64 dest)
{
    ...
    ma_subu(shift, shift, Imm32(32));
    ma_sll(ScratchRegister, dest.high, shift);
    as_or(dest.low, dest.low, ScratchRegister);
    ma_negu(shift, shift);
    ma_b(&done);

    ma_sra(ScratchRegister, dest.high, shift);
    as_or(dest.low, dest.low, ScratchRegister);
...
MacroAssembler::rshift64(Register unmaskedShift, Register64 dest)
{
    ...
    ma_srl(dest.low, dest.low, shift);
    ma_subu(shift, shift, Imm32(32));
    ma_srl(ScratchRegister, dest.high, shift);
    as_or(dest.low, dest.low, ScratchRegister);
    ma_negu(shift, shift);
    ma_sll(ScratchRegister, dest.high, shift);
    as_or(dest.low, dest.low, ScratchRegister);
...
MacroAssembler::branch64(Condition cond, Register64 lhs, Imm64 val, Label* success, Label* fail)
{
    ...
    switch(cond) {
      case Assembler::Equal:
        branch32(Assembler::NotEqual, lhs.low, val.firstHalf(), fail);
        branch32(Assembler::Equal, lhs.high, val.secondHalf(), success);
        if (!fallthrough)
            jump(fail);
        break;
      case Assembler::NotEqual:
        branch32(Assembler::NotEqual, lhs.low, val.firstHalf(), success);
        branch32(Assembler::NotEqual, lhs.high, val.secondHalf(), success);
        if (!fallthrough)
            jump(fail);
        break;
      ...
      case Assembler::AboveOrEqual: {
        ...
        Assembler::Condition cond2 = Assembler::ConditionWithoutEqual(Assembler::InvertCondition(cond));
        Assembler::Condition cond3 = Assembler::UnsignedCondition(cond);

        ma_b(lhs.high, val.secondHalf(), success, cond1);
        ma_b(lhs.high, val.secondHalf(), fail, cond2);
        ma_b(lhs.low, val.firstHalf(), success, cond3);

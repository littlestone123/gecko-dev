...
void
CodeGeneratorMIPS::visitMulI64(LMulI64* lir)
{
    ...
    if (IsConstant(rhs)) {
        ...
        switch (constant) {
          ...
          default:
            ...
            masm.mul64(Imm64(constant), output, temp);
        }
    }else{
        Register temp = ToTempRegisterOrInvalid(lir->temp());
        masm.mul64(ToOperandOrRegister64(rhs), output, temp);
    }
}
...
{
    MCompare* mir = lir->mir();
    MOZ_ASSERT(mir->compareType() == MCompare::Compare_Int64 || mir->compareType() == MCompare::Compare_UInt64);
...
void
CodeGeneratorMIPS::visitRotateI64(LRotateI64* lir)
{
    MRotate* mir = lir->mir();
...
void
CodeGeneratorMIPS::visitAsmJSPassStackArgI64(LAsmJSPassStackArgI64* ins)
{
...
void
CodeGeneratorMIPS::visitPopcntI64(LPopcntI64* lir)
{
    Register64 input = ToRegister64(lir->getInt64Operand(0));
    Register64 output = ToOutRegister64(lir);
...
CodeGeneratorMIPS::visitWasmLoadI64(LWasmLoadI64* lir)
{
    ...
    if (byteSize <= 4) {
        masm.ma_load(output.low, BaseIndex(HeapReg, ptr, TimesOne), static_cast<LoadStoreSize>(8 * byteSize), isSigned ? SignExtend : ZeroExtend);
        if ( !isSigned )
          masm.move32(Imm32(0), output.high);
        else
          masm.ma_sra(output.high, output.low, Imm32(31));
...
CodeGeneratorMIPS::visitWasmStoreI64(LWasmStoreI64* lir)
{
    ...
    MOZ_ASSERT(INT64LOW_OFFSET == 0);
    if (byteSize <= 4) {
        masm.ma_store(value.low, BaseIndex(HeapReg, ptr, TimesOne), static_cast<LoadStoreSize>(8 * byteSize));
    } else {
